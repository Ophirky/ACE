...
class UDPServerHandler:
    ...
    def receive_rtp_message(self) -> tuple | None:
        """
        ...
        while True:
            try:
                ...
                if decoded_packet.marker != CommunicationConsts.EXPECT_ANOTHER_FRAGMENT:
                    ...
                    message_data = (decoded_packet.timestamp,
                                    self.assemble_rtp_message(seq_start, decoded_packet.sequence_number))
                    if message_data:
                        break
...
#
# from rtp_parser import RTPPacketDecoder
# from src.server.utils.consts.logging_consts import *
# from src.server.utils.consts.tc_consts import CommunicationConsts, Ports
# from src.server.utils.logger import Logger
#
...
#         context.load_cert_chain(certfile=r"./docs/certificate.crt", keyfile=r"./docs/privateKey.key")
...
#         self.bind_address = CommunicationConsts.HOST
#         self.bind_port = port.value
#         self._unsafe_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#         self._unsafe_sock.bind((self.bind_address, self.bind_port))
#
#         # self.sock = context.wrap_socket(self._unsafe_sock, server_side=True)
#         self.sock = self._unsafe_sock
#
#         self._uncompleted_frame_packets = dict()
#
#         self.sock.settimeout(CommunicationConsts.FRAGMENT_RECEIVE_TIMEOUT)
...
#         return zlib.decompress(frame)
#
#     def receive_rtp_message(self) -> bytes | None:
#         """
#         Receives RTP packets from the client & decodes them.
#         :return bytes or None: If a message/packet is received then the payload will be returned.
#         """
#         message_data: None | bytes = None
...
#                                 struct.unpack('!I', decoded_packet.extension_data)[0] + 1
#
#                     message_queue.append((decoded_packet.timestamp, decoded_packet.payload))
#
#                     # Sort messages by timestamp and play in correct order
#                     message_queue.sort(key=lambda x: x[0])
#
#                     # Process messages from the queue
#                     while message_queue:
#                         ts, data = message_queue.pop(0)  # Get oldest message
#
